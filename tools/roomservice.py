...
default_team_rem = "Perferom"
# this shouldn't change unless google makes changes
local_manifest_dir = ".repo/local_manifests"
# change this to your name on github (or equivalent hosting)
android_team = "Perferom"
...
def fetch_dependencies(device):
    location = "vendor/perferom/dependencies/"
    ...
    dependencies = parse_dependency_file(location, False, True)
    create_dependency_manifest(dependencies, True)

def fetch_dependencies_via_location(location):
    if location is None or not os.path.isdir(location):
        raise Exception("ERROR: could not find your device "
                        "folder location, bailing out")
    dependencies = parse_dependency_file(location, True, False)
...
if __name__ == '__main__':
    ...
    if not os.path.isfile("vendor/perferom/dependencies/" + device + ".dependencies"):
            print("Dependency file for your device is missing.")
    else:
