...
function cmremote()
{
    ...
    GERRIT_REMOTE=$(cat .git/config | grep git://github.com/Perferom | awk '{ print $NF }' | sed s#git://github.com/##g)
    if [ -z "$GERRIT_REMOTE" ]
    then
        GERRIT_REMOTE=$(cat .git/config | grep http://github.com/Perferom | awk '{ print $NF }' | sed s#http://github.com/##g)
        if [ -z "$GERRIT_REMOTE" ]
        then
...
function upstream()
{
    ...
    GERRIT_REMOTE=$(echo $GERRIT_REMOTE | grep Perferom | awk '{ print $NF }' | sed s#Perferom#CyanogenMod#g)
